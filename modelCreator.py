...
class ModelCreator():
    ...
    def createForecastModels(self):
        for index, row in self.dfSensors.iterrows():
            ...
            dfYSensor = dbOperations.DbOperation.readFromAnalyticsCollection("Analytics", "devicetempshist",
                                                                             {"deviceId": index}, 1)
            ...
            for sensorEUI in PrevSensorsEUI:
                if (sensorEUI != "nan"):
                    indexS = sensorEUI
                    dfXSensor = dbOperations.DbOperation.readFromAnalyticsCollection("Analytics", "devicetempshist",
                                                                                     {"deviceId": indexS}, 1)
