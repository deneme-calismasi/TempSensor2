...
class ForecastCreator():
    ...
    def createForecast(self):
        for index, row in self.dfSensors.iterrows():
            ...
            dfYSensor = dbOperations.DbOperation.readLastFromAnalyticsCollection("Analytics", "devicetempshist",
                                                                                 {"deviceId": index}, 1)
            ...
            for sensorEUI in PrevSensorsEUI:
                if (sensorEUI != "nan"):
                    indexS = sensorEUI
                    dfXSensor = dbOperations.DbOperation.readLastFromAnalyticsCollection("Analytics", "devicetempshist",
                                                                                         {"deviceId": indexS}, 1)
